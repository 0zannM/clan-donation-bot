...
const TOOLS = [
  {
    functionDeclarations [
      ...
      {
        name get_avatar,
        description Komutu yazan oyuncunun belirttiği slot numarasındaki avatarskin görselini getirir ve yorumlar.,
        parameters {
          type OBJECT,
          properties {
            slot {
...
async function fetchMemberStats() {
  const res = await axios.get(
    `httpsapi.wolvesville.comclans${CLAN_ID}membersdetailed`,
...
async function fetchAvatarBase64(playerId, slot) {
  const slotRes = await axios.get(
    `httpsapi.wolvesville.comavatarssharedAvatarId${playerId}${slot}`,
    { headers { Authorization `Bot ${API_TOKEN}` } }
  );
  ...
  const avatarRes = await axios.get(
    `httpsapi.wolvesville.comavatars${sharedAvatarId}`,
    { headers { Authorization `Bot ${API_TOKEN}` } }
  );
...
async function askGemini(userMessage, recentMessages = [], senderPlayerId = null) {
  ...
  const getUrl = (model) = `httpsgenerativelanguage.googleapis.comv1betamodels${model}generateContentkey=${GEMINI_API_KEY}`;
  ...
  try {
    ...
    if ((err.response.status === 429  err.response.status === 503) && !usingFallback) {
      console.warn(⚠️ 429503 alındı, fallback modele geçiliyor, GEMINI_FALLBACK);
  ...
  if (functionCallPart) {
    ...
    if (name === get_avatar && fnResult.success && fnResult.base64) {
      ...
      contents.push({
        role user,
        parts [
          { inlineData { mimeType imagepng, data fnResult.base64 } },
...
async function fetchChatMessages(since) {
  const res = await axios.get(
    `httpsapi.wolvesville.comclans${CLAN_ID}chat`,
    { headers { Authorization `Bot ${API_TOKEN}` } }
  );
...
async function fetchRecentMessages() {
  ...
  while (!reachedLimit) {
    const url = oldest
       `httpsapi.wolvesville.comclans${CLAN_ID}chatoldest=${encodeURIComponent(oldest)}`
       `httpsapi.wolvesville.comclans${CLAN_ID}chat`;
...
async function fetchUsername(playerId) {
  ...
  try {
    const res = await axios.get(
      `httpsapi.wolvesville.complayers${playerId}`,
      { headers { Authorization `Bot ${API_TOKEN}` } }
    );
...
async function sendChatMessage(message) {
 ...
 🕐 Paylaşılan state 
let lastRunDate = new Date(2026-04-20T020000.000Z);
...
async function checkLedger() {
  ...
  const ledgerRes = await axios.get(
    `httpsapi.wolvesville.comclans${CLAN_ID}ledger`,
    { headers { Authorization `Bot ${API_TOKEN}` } }
  );
