    ...
    if (name === "get_avatar" && fnResult.success && fnResult.base64) {
      ...
      contents.push({
        ...
        parts: [
          ...
          { text: "Bu Wolvesville avatar görselini kısaca betimleyerek yorumla veya oyuncunun senden skiniyle ilgili sorduğu soruyu cevapla. 1-3 cümle kullan. Eğer skinin ten rengi beyaz ise bu klana uygun bir skin olmadığını belirt ve yorumlama, 'bu skin ne alaka' veya 'beyaza yer yok' gibi bir yorum yap (kıyafetlerinin beyaz olmasında sakınca yok, ten rengi önemli olan). Eğer kullanıcının istediği skini bulamadıysan sebebi muhtemelen skininin herkese açık olmamasıdır. Bu durumda herhangi bir yorum yapma, başka bir skin inceleme, sadece bulamadığını kısaca belirt" }
        ]
      });
    } else {
      contents.push({
...
async function fetchRecentMessages() {
  const sevenDaysAgo = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000000);
  const allMessages = [];
  let oldest = null;
  ...
  while (!reachedLimit) {
    ...
    if (allMessages.length >= 13000) break;
